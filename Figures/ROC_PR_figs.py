...
def plot_bar_plot(metric_values,std_values,p_values,feature_names, metric_name, plot_name,  fmt='.4f',
    ...
    for bar, mean,feature_name,std_val in zip(bars, metric_values,feature_names,std_values):
       # gap = (bar.get_height() - min_y)/2.0
        ...
        if feature_name!='Sequence only':
            ...
            max_height = min(max_y, bar.get_height() + 1.15*std_val)
            ax.text(bar.get_x() + bar.get_width()/2.0 ,max_height , annotation, va='center', ha='center', fontsize=12)
    # Aesthetics
    ...
    if additional_info:
        ...
        ax.legend(loc='upper left',fontsize = 11,borderaxespad=0.05,ncol=1)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    partition_information = {'Data set': 'TrueOT Lazzarotto','Cell type' : 'T cells', 'Verified OTSs': 73, 'Potential OTSs': 236583,'sgRNAs':6}
    ...
    # partition_information = {'Data set': 'TrueOT SP','Cell type' : 'CD34+ HSPC', 'Verified OTSs': 25, 'Potential OTSs': 81030,'sgRNAs':3}
